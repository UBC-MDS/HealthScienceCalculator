...
def test_bmr_edge_cases():
    ...
    expected = 111.6396  # Male: Near-zero inputs
    actual = get_bmr(1, 1, 1, "male")
    assert actual == pytest.approx(expected, 0.0001)

    expected = 455.9783  # Female: Near-zero inputs
    actual = get_bmr(1, 1, 1, "female")
    assert actual == pytest.approx(expected, 0.0001)

    # Extremely high values
    expected = 16034.3656  # Male: Large values
    actual = get_bmr(300, 250, 100, "male")
    assert actual == pytest.approx(expected, 0.0001)

    expected = 12022.8450  # Female: Large values
